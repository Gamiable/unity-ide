...
namespace Packages.IDE.Editor.ProjectGeneration
{
	internal class ProjectGeneration : IGenerator
	{
		...
		public bool SyncIfNeeded(IEnumerable<string> affectedFiles, IEnumerable<string> reimportedFiles, bool checkProjectFiles = false)
		{
			...
			if (HasFilesBeenModified(affectedFiles, reimportedFiles) || IDEScriptEditorData.instance.hasChanges
																															|| IDEScriptEditorData.instance.HasChangesInCompilationDefines()
		...
		public void Sync()
		{
			...
			_buffer = null;
			IDEScriptEditorData.instance.hasChanges = false;
			IDEScriptEditorData.instance.InvalidateSavedCompilationDefines();
		}
		...
		private void GenerateAndWriteSolutionAndProjects(Type[] types)
		{
			...
			foreach (var kvp in additionalAssetsByAssembly)
			{
				var assemblyName = kvp.Key;
				if (assemblyNamesWithSource.Contains(assemblyName))
					continue;

				var additionalAssets = kvp.Value;

				var executingAssemblyName = typeof(ProjectGeneration).Assembly.GetName().Name;
				var editorAssembly = m_AssemblyNameProvider.GetNamedAssembly(executingAssemblyName);
				string[] coreReferences = null;
				string rootNamespace = null;
				Assembly assembly = null;
				if (editorAssembly != null)
				{
					// We want to add those references, so that the editor would detect Unity path and version and provide rich features for shader files
					// Note that output path will be Library/ScriptAssemblies
					assembly = new Assembly(assemblyName, editorAssembly.outputPath, Array.Empty<string>(),
						new []{"UNITY_EDITOR"},
						Array.Empty<Assembly>(),
						coreReferences,
						editorAssembly.flags
				...
				projectParts.Add(new ProjectPart(assemblyName, assembly, additionalAssets));
			}

		SyncSolution(new StringBuilder(), projectParts, types);

		foreach (var projectPart in projectParts)
		{
			SyncProject(new StringBuilder(), projectPart, assemblyUsage, types);
		}
		...
		private Dictionary<string, List<string>> GetAdditionalAssets()
		{
			...
			foreach (var asset in interestingAssets)
			{
				...
				// add a file, but we could also "Show All Files" and do the same. Equally, the editor defaults to the Unity Explorer
		...
		{
			return TypeCache.GetTypesDerivedFrom<AssetPostprocessor>().ToArray();
		}
					...
					if (assemblyUsage.IsProjectAssembly(reference))
{
	...
	private void GenerateVSCodeSettings()
	{
		if (IDEScriptEditor.ExecutableStartsWithAny(IDEScriptEditor.CurrentEditor, "nv"))
			return;

		var vscodeDir = Path.Combine(ProjectDirectory, ".vscode");
		var settingsPath = Path.Combine(vscodeDir, "settings.json");

		if (m_FileIOProvider.Exists(settingsPath))
			return;

		if (!m_FileIOProvider.Exists(vscodeDir))
			Directory.CreateDirectory(vscodeDir);

		var sb = new StringBuilder();
		sb.AppendLine("{");
		sb.AppendLine("    \"files.exclude\": {");
		sb.AppendLine("        \"**/.DS_Store\": true,");
		sb.AppendLine("        \"**/.git\": true,");
		sb.AppendLine("        \"**/.gitmodules\": true,");
		sb.AppendLine("        \"**/*.booproj\": true,");
		sb.AppendLine("        \"**/*.pidb\": true,");
		sb.AppendLine("        \"**/*.suo\": true,");
		sb.AppendLine("        \"**/*.user\": true,");
		sb.AppendLine("        \"**/*.userprefs\": true,");
		sb.AppendLine("        \"**/*.unityproj\": true,");
		sb.AppendLine("        \"**/*.dll\": true,");
		sb.AppendLine("        \"**/*.exe\": true,");
		sb.AppendLine("        \"**/*.pdf\": true,");
		sb.AppendLine("        \"**/*.mid\": true,");
		sb.AppendLine("        \"**/*.midi\": true,");
		sb.AppendLine("        \"**/*.wav\": true,");
		sb.AppendLine("        \"**/*.gif\": true,");
		sb.AppendLine("        \"**/*.ico\": true,");
		sb.AppendLine("        \"**/*.jpg\": true,");
		sb.AppendLine("        \"**/*.jpeg\": true,");
		sb.AppendLine("        \"**/*.png\": true,");
		sb.AppendLine("        \"**/*.psd\": true,");
		sb.AppendLine("        \"**/*.tga\": true,");
		sb.AppendLine("        \"**/*.tif\": true,");
		sb.AppendLine("        \"**/*.tiff\": true,");
		sb.AppendLine("        \"**/*.3ds\": true,");
		sb.AppendLine("        \"**/*.3DS\": true,");
		sb.AppendLine("        \"**/*.fbx\": true,");
		sb.AppendLine("        \"**/*.FBX\": true,");
		sb.AppendLine("        \"**/*.lxo\": true,");
		sb.AppendLine("        \"**/*.LXO\": true,");
		sb.AppendLine("        \"**/*.ma\": true,");
		sb.AppendLine("        \"**/*.MA\": true,");
		sb.AppendLine("        \"**/*.obj\": true,");
		sb.AppendLine("        \"**/*.OBJ\": true,");
		sb.AppendLine("        \"**/*.asset\": true,");
		sb.AppendLine("        \"**/*.cubemap\": true,");
		sb.AppendLine("        \"**/*.flare\": true,");
		sb.AppendLine("        \"**/*.mat\": true,");
		sb.AppendLine("        \"**/*.meta\": true,");
		sb.AppendLine("        \"**/*.prefab\": true,");
		sb.AppendLine("        \"**/*.unity\": true,");
		sb.AppendLine("        \"build/\": true,");
		sb.AppendLine("        \"Build/\": true,");
		sb.AppendLine("        \"Library/\": true,");
		sb.AppendLine("        \"library/\": true,");
		sb.AppendLine("        \"obj/\": true,");
		sb.AppendLine("        \"Obj/\": true,");
		sb.AppendLine("        \"ProjectSettings/\": true,");
		sb.AppendLine("        \"temp/\": true,");
		sb.AppendLine("        \"Temp/\": true,");
		sb.AppendLine("        \"Logs/\": true");
		sb.AppendLine("    }");
		sb.AppendLine("}");

		m_FileIOProvider.WriteAllText(settingsPath, sb.ToString());
	}

	private void GenerateVSCodeLaunch()
	{
		var vscodeDir = Path.Combine(ProjectDirectory, ".vscode");
		var launchPath = Path.Combine(vscodeDir, "launch.json");

		if (m_FileIOProvider.Exists(launchPath))
			return;
		...
		sb.AppendLine("        {");
		sb.AppendLine("            \"name\": \"Attach to Unity Editor\",");
		sb.AppendLine("            \"type\": \"unity\",");
		sb.AppendLine("            \"request\": \"attach\"");
		sb.AppendLine("        },");
		sb.AppendLine("        {");
		sb.AppendLine("            \"name\": \"Attach to Unity Player\",");
		sb.AppendLine("            \"type\": \"unity\",");
		sb.AppendLine("            \"request\": \"attach\",");
		sb.AppendLine("            \"transportArgs\": {");
		sb.AppendLine("                \"port\": 56000");
		sb.AppendLine("            }");
		sb.AppendLine("        }");
		sb.AppendLine("    ]");
		sb.AppendLine("}");

		m_FileIOProvider.WriteAllText(launchPath, sb.ToString());
	}

	private void GenerateNvproj()
	{
		if (!IDEScriptEditor.ExecutableStartsWithAny(IDEScriptEditor.CurrentEditor, "nv"))
			return;

		var nvprojPath = Path.Combine(ProjectDirectory, ".nvproj");

		if (m_FileIOProvider.Exists(nvprojPath))
			return;
		...
		sb.AppendLine("\t\tfiles = {");
		sb.AppendLine("\t\t\t\".gitmodules\", \"*.meta\",");
		sb.AppendLine("\t\t\t\"*.booproj\", \"*.pidb\", \"*.suo\",");
		sb.AppendLine("\t\t\t\"*.user\", \"*.userprefs\", \"*.unityproj\",");
		sb.AppendLine("\t\t\t\"*.dll\", \"*.exe\", \"*.so\", \"*.a\", \"*.bundle\",");
		sb.AppendLine("\t\t\t\"*.pdf\", \"*.mid\", \"*.midi\", \"*.wav\",");
		sb.AppendLine("\t\t\t\"*.gif\", \"*.ico\", \"*.jpg\", \"*.jpeg\", \"*.png\",");
		sb.AppendLine("\t\t\t\"*.psd\", \"*.tga\", \"*.tif\", \"*.tiff\",");
		sb.AppendLine("\t\t\t\"*.3ds\", \"*.3DS\", \"*.fbx\", \"*.FBX\",");
		sb.AppendLine("\t\t\t\"*.lxo\", \"*.LXO\", \"*.ma\", \"*.MA\",");
		sb.AppendLine("\t\t\t\"*.obj\", \"*.OBJ\", \"*.ttf\",");
		sb.AppendLine("\t\t\t\"*.cubemap\", \"*.flare\", \"*.mat\",");
		sb.AppendLine("\t\t\t\"*.prefab\", \"*.unity\", \"*.mdb\", \"*.blend\",");
		sb.AppendLine("\t\t\t\"*.csproj\", \"*.sln\",");
		sb.AppendLine("\t\t},");
		sb.AppendLine("\t\tdirs = {");
		...
		sb.AppendLine("\t\t\t\"ProjectSettings\", \"Logs\",");
		sb.AppendLine("\t\t},");
		sb.AppendLine("\t},");
		sb.AppendLine("}");

		m_FileIOProvider.WriteAllText(nvprojPath, sb.ToString());
	}
}
