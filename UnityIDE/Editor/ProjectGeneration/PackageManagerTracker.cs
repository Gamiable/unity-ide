	...
	internal static class PackageManagerTracker
	{
		private static bool HasManifestJsonLastWriteTimeChanged()
		{
			...
			if (manifestFile.Exists)
			{
				...
				var res = manifestFile.LastWriteTime > IDEScriptEditorPersistedState.instance.ManifestJsonLastWrite;
				if (res) IDEScriptEditorPersistedState.instance.ManifestJsonLastWrite = manifestFile.LastWriteTime;
				return res;
			}
		...
		/// <summary>
		/// If the manifest.json was changed outside Unity and the editor calls Unity to Refresh, we should call PM to Refresh its state also
		/// </summary>
