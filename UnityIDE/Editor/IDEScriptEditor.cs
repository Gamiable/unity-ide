...
#pragma warning restore 618
		...
		private static bool OpenFileInIDE(string editorPath, string filePath, int line, int column)
		{
			...
			try
			{
				...
				var isMacApp = editorPath.EndsWith(".app", StringComparison.OrdinalIgnoreCase);
				...
				if (isMacApp && !hasGoto)
				{
					var process = Process.Start("open", $"-a \"{editorPath}\" \"{projectDir}\"");
					process?.WaitForExit(5000);
					return process?.ExitCode == 0;
				}

				var executablePath = isMacApp
					? GetMacExecutablePath(editorPath)
					: editorPath;
				...
				var proc = Process.Start(executablePath, arguments);
		...
		public static bool IsSupportedInstallation(string path)
		{
			...
			return ExecutableStartsWithAny(path, "code", "cursor", "windsurf", "antigravity", "zed", "subl", "sublime_text", "neovim", "nv");
		}
		...
		public static string GetEditorDisplayName(string path)
		{
			...
			if (filename.StartsWith("subl", StringComparison.OrdinalIgnoreCase)) return "Sublime Text";
